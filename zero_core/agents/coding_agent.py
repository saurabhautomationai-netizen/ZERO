"""Coding Agent for ZERO (Milestone M16 & Autonomous Builder Pipeline).

Provides code inspection, structural analysis, AST parsing, refactoring diff generation,
and autonomous test runner capabilities.
"""
...
class TestExecutionResult:
    """Represents the execution result of automated test suite."""
    success: bool
    exit_code: int
    output: str
    target_dir: str
...
class CodingAgent:
    """Native Coding Agent analyzing code structure, generating code, and running test suites."""
    ...
    def generate_module_stub(self, module_name: str, class_name: str, methods: List[str]) -> str:
        """Produces a clean typed Python module template."""
        method_defs = []
        for m in methods:
            method_defs.append(f"    def {m}(self, *args, **kwargs) -> Any:\n        \"\"\"Executes {m}.\"\"\"\n        pass\n")

        return (
            f'"""Module {module_name} generated by ZERO Coding Agent."""\n\n'
            f"from __future__ import annotations\n\n"
            f"from typing import Any, Dict, List, Optional\n\n\n"
            f"class {class_name}:\n"
            f"    \"\"\"Handles {class_name} domain logic.\"\"\"\n\n"
            + "\n".join(method_defs)
        )

    def run_project_tests(self, project_dir: Path) -> TestExecutionResult:
        """Runs pytest on a target project directory and returns test execution results."""
        if not project_dir.exists():
            return TestExecutionResult(
                success=False,
                exit_code=-1,
                output=f"Target directory {project_dir} does not exist.",
